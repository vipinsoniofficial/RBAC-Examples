class PermissionMixin(object):
    ...
    def __init__(self, perm=None):
        ...
        if not hasattr(self.__class__, 'children'):
            self.children = set()
        PermissionMixin.permissions[perm] = self
    ...
    @staticmethod
    def get_by_permission(perm):
        """A static method to return the permission which has the input name.

        :param perm: type of permission
        """
        return PermissionMixin.permissions[perm]


class RoleMixin(object):
    roles = {}

    def __init__(self, name=None, permissions=[]):
        self.name = name
...
Read = PermissionMixin('anonymous')
anonymous = RoleMixin('anonymous', permissions=[Read])
