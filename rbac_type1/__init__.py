...
from rbac_type2.rbac_build.model_example import RoleMixin, UserMixin, anonymous
    ...
    def allow(self, role, method, resource, with_children=True):
        ...
        if with_children:
            for r in role.get_children():
                permission = (r.get_name(), r.get_permission(), method, resource)
                if not permission in self._allowed:
                    self._allowed.append(permission)
        permission = (role.get_name(), role.get_permission(), method, resource)
        if not permission in self._allowed:
            self._allowed.append(permission)
...
class RBAC(object):
    ...
    def has_permissions(self, permission, endpoint, user=None):
        """Return does the current user can access the resource.
        Example::
        `rbac.has_permission('READ', 'a_view_func')` return True.
        `rbac.has_permission('WRITE', 'a_view_func')` return False.

        :param permission: The method wait to check.
        :param endpoint: The application endpoint.
        :param user: user who you need to check. Current user by default.
        """
        app = self.get_app()
        _user = user or self._user_loader()
        permissions = _user.get_permissions()
        view_func = app.view_functions[endpoint]
        return self._check_perm(permissions, permission, view_func)
    ...
    def allow_permissions(self, permissions, methods, with_children=True):
        """This is a decorator function.
        ...
        An example::
            ...
            @app.route('/website/setting', methods=['GET', 'POST'])
            @rbac.allow_permissions(['Read', 'Write'], ['GET', 'POST'])
            def website_setting():
                return Response('Setting page.')

        :param permission: List, each name of permission.
        :param methods: List, each name of methods.
                        methods is valid in ['GET', 'POST', 'PUT', 'DELETE']
        :param with_children: Whether allow children of roles as well.
                              True by default.
        ...
        def decorator(view_func):
            _methods = [m.upper() for m in methods]
            for r, m, v in itertools.product(permissions, _methods, [view_func]):
                self.before_acl['allow'].append((r, m, v, with_children))
            return view_func
        return decorator
    ...
    def deny_permissions(self, permissions, methods, with_children=False):
        """This is a decorator function.
    ...
    def _check_perm(self, permissions, permission, resource):
        if self.acl.is_exempt(resource):
            return True
        ...
        _permissions = set()
        _perm = set(['*', permission])
        _resources = set([None, resource])

        if self.use_white:
            _permissions.add(anonymous)

        is_allowed = None
        _permissions.update(permissions)

        for r, m, res in itertools.product(_permissions, _perm, _resources):
            if self.acl.is_denied(r.get_permission(), m, res):
                return False

            if self.acl.is_allowed(r.get_permission(), m, res):
                is_allowed = True
